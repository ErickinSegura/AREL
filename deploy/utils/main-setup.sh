...
while ! state_done MTDR_DB_OCID; do
  MTDR_DB_OCID=`oci db autonomous-database list --compartment-id "$(cat ../state/COMPARTMENT_OCID)" --query "data[?\"display-name\"=='MTDRDB'] | sort_by(@, &\"time-created\") | [-1].id" --raw-output`
